...
本文位于 [[02-AISystem/cluster-and-hardware/00-Overview-GPU服务器：从物理硬件到Linux设备系统|GPU 服务器多层架构]] 的 PCIe Fabric 层，承接 [[02-AISystem/cluster-and-hardware/06-MSI与MSI-X：从数据完成到中断通知|MSI/MSI-X]]。前几篇回答了“设备怎样被发现、映射和传输数据”；本篇回答“链路怎样保持可用，以及发生错误后系统如何记录、隔离、复位和恢复”。因此，“BDF 仍在 `/sys`”只能证明设备对象尚在，不能证明链路、事务和驱动数据面仍然健康。
